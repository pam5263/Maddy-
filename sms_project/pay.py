...
def get_config(key, default=None):
    # Try to read from a local config.txt file first, then environment
...
VONAGE_API_KEY = get_config("VONAGE_API_KEY")
VONAGE_API_SECRET = get_config("VONAGE_API_SECRET")
VONAGE_PHONE_NUMBER = get_config("VONAGE_PHONE_NUMBER")
AWS_ACCESS_KEY_ID = get_config("AWS_ACCESS_KEY_ID")
AWS_SECRET_ACCESS_KEY = get_config("AWS_SECRET_ACCESS_KEY")
AWS_REGION_NAME = get_config("AWS_REGION_NAME")
PLIVO_AUTH_ID = get_config("PLIVO_AUTH_ID")
PLIVO_AUTH_TOKEN = get_config("PLIVO_AUTH_TOKEN")
PLIVO_PHONE_NUMBER = get_config("PLIVO_PHONE_NUMBER")
MESSAGEBIRD_API_KEY = get_config("MESSAGEBIRD_API_KEY")
MESSAGEBIRD_PHONE_NUMBER = get_config("MESSAGEBIRD_PHONE_NUMBER")
TELNYX_API_KEY = get_config("TELNYX_API_KEY")
TELNYX_PHONE_NUMBER = get_config("TELNYX_PHONE_NUMBER")
TELESIGN_CUSTOMER_ID = get_config("TELESIGN_CUSTOMER_ID") or get_config("TELESIGN_AUTH_ID")
TELESIGN_API_KEY = get_config("TELESIGN_API_KEY") or get_config("TELESIGN_AUTH_TOKEN")
TELESIGN_PHONE_NUMBER = get_config("TELESIGN_PHONE_NUMBER")
TEXTBELT_API_KEY = get_config("TEXTBELT_API_KEY")
...
def send_sms_telesign(phone_number, message, proxy=None):
    global TELESIGN_CUSTOMER_ID, TELESIGN_API_KEY, telesign_client
    try:
        if not all([TELESIGN_CUSTOMER_ID, TELESIGN_API_KEY]):
            print("\n\033[1;31m[!] Telesign credentials not configured.\033[0m")
            TELESIGN_CUSTOMER_ID = input("Enter TELESIGN CUSTOMER ID: ").strip()
            TELESIGN_API_KEY = input("Enter TELESIGN API KEY: ").strip()
            if not all([TELESIGN_CUSTOMER_ID, TELESIGN_API_KEY]):
                raise ConnectionError("Telesign credentials not configured.")
            set_config("TELESIGN_CUSTOMER_ID", TELESIGN_CUSTOMER_ID)
            set_config("TELESIGN_API_KEY", TELESIGN_API_KEY)
            telesign_client = MessagingClient(TELESIGN_CUSTOMER_ID, TELESIGN_API_KEY)
...
def configure_apis():
    ...
    while True:
        print("\n--- API Settings ---")
        print(f"1. Telesign Customer ID: {TELESIGN_CUSTOMER_ID}")
        print(f"2. Telesign API Key: {'*' * len(TELESIGN_API_KEY) if TELESIGN_API_KEY else 'None'}")
        print("3. Back to Main Menu")
        choice = input("Select: ")
        if choice == '1':
            TELESIGN_CUSTOMER_ID = input("Enter Telesign Customer ID: ").strip()
            ...
            if all([TELESIGN_CUSTOMER_ID, TELESIGN_API_KEY]):
                telesign_client = MessagingClient(TELESIGN_CUSTOMER_ID, TELESIGN_API_KEY)
            break
